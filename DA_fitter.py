...
new_best=optimize.fmin(fitting_scripts.fit_func_test,(first_T,first_g,10.),args=(spec_n,l_crop,model_c,0),disp=0,xtol=1.,ftol=1.,full_output=1)
other_T=optimize.fmin(fitting_scripts.err_t,(first_T,first_g),args=(new_best[0][2],new_best[1],spec_n,l_crop,model_c),disp=0,xtol=1.,ftol=1.,full_output=0)
...
lines_s,lines_m,model_n=fitting_scripts.fit_func_test((best_T,best_g,best_rv),spec_n,l_crop,models=model_c,mode=1)
#repeat fit for secondary solution
second_best=optimize.fmin(fitting_scripts.fit_func_test,(other_sol[0][0],other_sol[0][1],best_rv),args=(spec_n,l_crop,model_c,0),disp=0,xtol=1.,ftol=1.,full_output=1)
other_T2=optimize.fmin(fitting_scripts.err_t,(other_sol[0][0],other_sol[0][1]),args=(best_rv,second_best[1],spec_n,l_crop,model_c),disp=0,xtol=1.,ftol=1.,full_output=0)
...
# get and save best model
lines_s_o,lines_m_o,model_n_o=fitting_scripts.fit_func_test((s_best_T,s_best_g,s_best_rv),spec_n,l_crop,models=model_c,mode=1)
...
if plot == True:
    ...
    check_f_spec=spectra[:,1][(spectra[:,0]>4500.) & (spectra[:,0]<4700.)]
    check_f_model=model_n[:,1][(model_n[:,0]>4500.) & (model_n[:,0]<4700.)]
    adjust=np.average(check_f_model)/np.average(check_f_spec)
    ax2.plot(model_n[:,0]*(best_rv+c)/c,model_n[:,1]/adjust,color='r')
    check_f_model_o=model_n_o[:,1][(model_n_o[:,0]>4500.) & (model_n_o[:,0]<4700.)]
